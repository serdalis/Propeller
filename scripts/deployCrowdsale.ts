...
async function main() {
  ...
  const PropToken = await ethers.getContractFactory("PropToken", deployer);
  const propeller: Propeller = PropToken.attach(propTokenAddress) as Propeller;
  
  const blockNumBefore = await ethers.provider.getBlockNumber();
  const blockBefore = await ethers.provider.getBlock(blockNumBefore);
  const timestampBefore = blockBefore.timestamp;

  const PropellerCrowdsale = await ethers.getContractFactory("PropTokenCrowdsale", deployer);
  ...
  await propeller.mint(propellerCrowdsale.address, crowdsaleTokenCount);
